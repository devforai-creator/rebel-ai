...
| Supported secondary / fallback | `summary_window`, summary generation that supports the maintained default or system fallback, local-only maintainer RBX import for oversized archives           | Maintained because it supports the active defaults or operator reality, even if it is not always the main interactive path | Durable triage is required. Escalate to health degradation when repeated failures threaten the maintained fallback contract |
