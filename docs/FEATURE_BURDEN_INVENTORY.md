...
Source of truth: code, tests, migrations, generated types, and the active deployment
Revisit when: first-class product mode, provider strategy, memory strategy, or public/private surface changes
...
| Alternate models / secondary API key mode            | Actively used maintainer workflow for alternating primary/secondary keys during chat.                                                         | Chat settings state, persisted model config, UI controls, tests.                                                                                                              | `supported-secondary`                             | Keep the current workflow, but later decide how it should relate to response-variant/model comparison.                                       |
...
| Announcements                                        | Admin-operated banner surface from the earlier possible-public-service product assumption.                                                    | Admin flag, announcements table, API/UI.                                                                                                                                      | `parked public-service residue`                   | Preserve for now, but consider hiding it from the closed personal deployment until public mode returns.                                      |
| Feedback box                                         | Dashboard feedback capture from the earlier possible-public-service product assumption.                                                       | `user_feedback` table and dashboard form.                                                                                                                                     | `parked public-service residue`                   | Preserve for now, but consider hiding it from the closed personal deployment until there are outside users to hear from.                     |
| Provider expansion: DeepSeek / OpenRouter            | Broadens BYOK provider choice beyond the original Google/OpenAI/Anthropic surface and supports low-cost RP through cheaper/open model access. | Provider catalog, model factory, key rules, pricing, tests.                                                                                                                   | `supported-secondary`                             | Keep provider breadth that supports active use, lower-cost RP, or BYOK identity; avoid treating provider count as free.                      |
...
5. Keep alternate models as a supported secondary maintainer workflow, while
   later clarifying its relationship to future variant/model comparison.
6. Treat announcements and feedback as parked public-service residue unless
   public mode returns or closed deployment use proves they still earn UI space.
7. Keep provider breadth as supported secondary when it materially helps RP cost,
   model choice, or BYOK identity.
8. Keep optional provider-cache work behind removable adapters, not product
   promises.
